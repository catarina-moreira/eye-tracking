...
class MIMICDataProcessor:
  ...
  def processEYE_GAZE(self, global_vars : GlobalVars):
    ...
    for indx in range(0,metadata.shape[0]):
      ...
      patient_data = {}
      patient_data = self.extractIdentifiersEYEGAZE( metadata, indx, global_vars )
      ...
      print("Finished processing patient " + str(global_vars.getTOTAL_EYE_GAZE()) + "/ 1083")
  ...
  def extractEYEGAZEDataFromXAMI(self, metadata : pd.DataFrame, patient_data : dict, indx : int, global_vars : GlobalVars ):
    ...
    self.getImageInformation(patient_data, global_vars)
    # add audio information from radiologist's readings
    self. getAudioInformation( patient_data )
    # get fixation points data
    self.getEyeGazeData(patient_data, global_vars)
    # extract bounding boxes
    self.getPatientsBoundingBoxes(patient_data, global_vars)


  def getPatientsBoundingBoxes(self, patient_data : dict, global_vars : GlobalVars):
  ...
  def generatePyGazePaths(self, patient_id : str, patient_folder : str, image_id : str, plot_type : str, global_vars : GlobalVars):
  ...
  def getEyeGazeData(self, patient_data : dict, global_vars :GlobalVars ):
    ...
    temp_dict = {}
    temp_dict['PUPILS'] = self.generatePyGazePaths(patient_id, patient_folder, image_id, "Pupils", global_vars)
    temp_dict['FIXATIONS'] = self.generatePyGazePaths(patient_id,patient_folder, image_id, "Fixations", global_vars)
    temp_dict['GAZE'] = self.generatePyGazePaths(patient_id,patient_folder, image_id, "Gaze", global_vars)
  ...
  def getImageInformation(self, patient_data : dict, global_vars : GlobalVars):
    patient_folder = "patient_" + patient_data["PATIENT_ID"] 
    image_ID = patient_data['IMAGE_ID']
  ...
  def extractIdentifiersEYEGAZE(self, metadata, indx, global_vars ):
